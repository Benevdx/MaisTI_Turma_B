...
const servidor = http.createServer((req, res) => {
    ...
    console.log(`Requisição recebida: Data -> ${dataAtual}; método -> ${req.method}; URL -> ${req.url}`);
    ...
    else if(req.method === 'GET' && req.url.startsWith('/saudacoes')){
        const nome = reqUrl.query.nome;

        res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});

        if(nome){
            res.end(`<h1>Olá, ${nome}!</h1>`);
        } else{
            res.end('<h1>Olá! Informe um nome na URL, ex: /saudacoes?nome=Calebe</h1>');
        }
    }
